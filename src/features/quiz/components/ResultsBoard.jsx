  ...
  return (
    <div className="w-full max-w-5xl mx-auto flex flex-col items-center pb-32 pt-12 md:pt-16 px-6 font-sans relative">
      ...
      <div className="w-full flex flex-col items-center">
        <h1 className="text-5xl md:text-7xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 tracking-tighter mb-10 text-center drop-shadow-[0_0_20px_rgba(0,210,255,0.4)]">
          Assessment Results
        </h1>
        
        <div className="w-full max-w-xl bg-[rgba(15,20,30,0.6)] backdrop-blur-2xl border border-[rgba(255,255,255,0.1)] rounded-3xl p-10 md:p-14 shadow-[0_0_50px_rgba(0,0,0,0.5)] relative overflow-hidden">
          <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-cyan-500/50 to-transparent"></div>

          <div className="flex flex-col gap-6 mb-12">
            <div className="flex justify-between items-center bg-[rgba(0,0,0,0.4)] p-5 rounded-2xl border border-[rgba(255,255,255,0.05)] shadow-inner">
              <span className="text-base font-bold text-gray-400 tracking-widest uppercase">Total Questions</span>
              <span className="text-3xl md:text-4xl font-black text-white tabular-nums drop-shadow-md">{total}</span>
            </div>
            
            <div className="flex justify-between items-center bg-[rgba(0,0,0,0.4)] p-5 rounded-2xl border border-[rgba(255,255,255,0.05)] shadow-inner">
              <span className="text-base font-bold text-gray-400 tracking-widest uppercase">Correct Answers</span>
              <span className="text-3xl md:text-4xl font-black text-green-400 tabular-nums drop-shadow-[0_0_10px_rgba(34,197,94,0.6)]">{correct}</span>
            </div>
            
            <div className="flex justify-between items-center bg-[rgba(0,0,0,0.4)] p-5 rounded-2xl border border-[rgba(255,255,255,0.05)] shadow-inner">
              <span className="text-base font-bold text-gray-400 tracking-widest uppercase">Accuracy</span>
              <span className="text-4xl md:text-5xl font-black text-cyan-400 tabular-nums neon-text-blue">{accuracy}%</span>
            </div>
          </div>
      ...
      <div className="mt-20 w-full max-w-4xl font-sans">
        ...
        <div className="flex flex-col gap-8">
          {firstAttemptAnswers.map((item, idx) => (
            <div key={idx} className={`bg-[rgba(15,20,30,0.6)] border-l-4 ${item.isCorrect ? 'border-l-green-500' : 'border-l-red-500'} border-y border-r border-[rgba(255,255,255,0.05)] rounded-r-2xl rounded-l-md p-6 md:p-10 backdrop-blur-2xl shadow-[0_10px_30px_rgba(0,0,0,0.3)] relative overflow-hidden`}>
